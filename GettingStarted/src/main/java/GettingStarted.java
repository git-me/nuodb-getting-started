	...
	private static AtomicLong counter = new AtomicLong(0);
		...
		int teId = -1;
		    ...
		    log.info(String.format("Ran 100 queries in task %d on TE %d; total=%d", id, teId, current));
                ...
                } catch (SQLTransientConnectionException teFailover) {
                    log.info(String.format("TE communication error on %d - failing over to a new TE: %s", teId, teFailover.toString()));
                } catch (SQLNonTransientConnectionException nonTransientFailure) {
                    log.info(String.format("Error making initial connection - retrying: %s", nonTransientFailure.toString()));
